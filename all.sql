...
select count(*) from case2.tbl_user;
select * from case2.tbl_data;
select count(*) from case3.tbl_data;




-- 鼠标点击某个节点时显示详细信息
select * from case2.tbl_user where UserID = 3;
...
-- 比如查询日期为2016-11-04的所有用户通话记录
select distinct (select UserID from tbl_user where PhoneNumber = tbl_user.PhoneNum) as UserID,(select UserID from tbl_user where OppositePhoneNumber = tbl_user.PhoneNum) as OppositeID from tbl_data where CallDate = "2016-11-04";

-- 查询时间00:00到05:00之间所有通话记录
select distinct (select UserID from tbl_user where PhoneNumber = tbl_user.PhoneNum) as UserID,(select UserID from tbl_user where OppositePhoneNumber = tbl_user.PhoneNum) as OppositeID from tbl_data where CallTime > "00:00:00" and CallTime < "05:00:00";

-- 查询日期为2016-11-03，通话时间在00:00-05:00之间，地域为兰州的所有通话记录
select * from tbl_data where CallDate = "2016-11-03" and CallTime > "00:00:00" and CallTime < "05:00:00" and PhoneArea like "兰州%";
